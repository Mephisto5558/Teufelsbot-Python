...
def create_category_component(interaction, lang, command_categories: list | None = None):
  if not command_categories:
    command_categories = list(set([
        e.category for e in list(interaction.client.prefix_commands.values())
        + list(interaction.client.slash_commands.values())
    ]))
  ...
  if cmd.cooldowns.user or cmd.cooldowns.guild:
    ...
    for k, v in cmd.cooldowns.items():
      if v:
        min_ = v // 60000
        sec = (v % 60000) / 1000
        sec = int(sec) if sec % 1 == 0 else round(sec, 2)
...
def all_query(interaction, lang):
  command_categories = list(set(e.category for e in get_all_commands(interaction)))
  if interaction.user.id != interaction.client.application.id:
    command_categories = [(e for e in command_categories if not e.lower() in owner_only_folders)]
...
def command_query(interaction, lang, query: str):
  if len(interaction.values) == 0: return category_query(interaction, lang, interaction.message.components[0].components[0].data.options.find(e => e.default).value)
  ...
  help_lang = partial(I18nProvider.__, none_not_found=True, locale=interaction.guild.locale_code, backup_path=f'commands.{command.category.lower()}.{command.name}')
  embed = EmbedBuilder(
    ...
    footer={'text': lang('one.embedFooterText', interaction.guild.db['config.prefix'] or interaction.client.defaultSettings['config.prefix'])},
    color=Colors.Blurple,
  )
